...
The remainder of the paper presents the measurement framework (\S\ref{sec:method}), the eight robustness tests including the held-out validation and trace-level mechanism (\S\ref{sec:experiments}), and the prototype diagnostic tool AgentDiff-Probe v2 (\S\ref{sec:probe}); a detailed list of limitations follows the conclusion.
...
\paragraph{Stealth divergence + retractions.} M3 and M4 paint a single picture: under a semantic-preserving rewrite the agent commits to the same first action as on the original input, but its intermediate thoughts drift further before resyncing. Surface edits, by contrast, produce \emph{loud} early divergences that the agent can ignore or correct. A prior 26-cell analysis reported sem diverging earlier (M1, $t{=}-2.30$, $p{=}0.021$) and self-correcting $3{\times}$ less often (M2, $p{=}0.005$); on the held-out run M1 reverses sign and M2 collapses ($p{=}0.230$). We retract both and base the mechanism account on M3 and M4.

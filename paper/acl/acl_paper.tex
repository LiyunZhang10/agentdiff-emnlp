...
\begin{enumerate}
\item \textbf{A directional inconsistency gap robust across four severity definitions and family-level subsampling.} Across the 68-cell panel the mean gap is $+11.90$~pp (paired $t=8.11$, $p<0.0001$; 54/68 positive); the gap is stable under token Jaccard, Sentence-BERT cosine, and length-change ratio severity proxies, all $p<0.0001$ (\S\ref{sec:severity}, Table~\ref{tab:severity_proxies}). On the 48 non-qwen cells alone it is $+11.10$~pp ($p<0.0001$); the GSM8K cascade gap survives a TF-IDF redefinition ($+0.66$ steps, $p<0.001$).
...
\caption{Per-cell $\Delta$ across 68 cells of the 10-model panel; 54/68 are positive, with a mean of $+11.90$~pp (paired $t=8.11$, $p<0.0001$).}
\label{fig:fig1}
\end{figure}
...
\midrule
Mean across 68 cells (pp) & $+11.90$ & --- \\
Cells with $\Delta>0$     & 54 / 68 & --- \\
Median (pp)               & $+9.57$ & --- \\
Paired $t$ vs.\ zero, $t$ & $8.11$ & --- \\
Paired $t$, $p$           & $<10^{-4}$ & $<10^{-4}$ \\
Wilcoxon, $p$             & $<10^{-4}$ & $<10^{-4}$ \\
\bottomrule
\end{tabular}
\caption{Inconsistency rate gap $\Delta$ across the 68 cells (10 models $\times$ 3 benchmarks $\times$ 2 scaffolds, plus 8 generator-swap cells). The $\Delta_{\text{matched}}$ column is pending re-run of the severity-matching pipeline on the expanded panel; on the original 44-cell subset, the matched $\Delta$ was within $0.3$~pp of the raw value.}
...
\midrule
Edit distance, normalised      & $+14.85$ & $7.68$ & $<10^{-4}$ & 39/44$^{\dagger}$ \\
Token Jaccard distance         & $+15.44$ & $7.22$ & $<10^{-4}$ & 37/44$^{\dagger}$ \\
Sentence-BERT cosine distance  & $+13.72$ & $6.75$ & $<10^{-4}$ & 40/44$^{\dagger}$ \\
Absolute length-change ratio   & $+14.06$ & $7.07$ & $<10^{-4}$ & 38/44$^{\dagger}$ \\
\bottomrule
\end{tabular}
\caption{Severity-matched $\Delta$ under four severity proxies. $^{\dagger}$Values shown are from the original 44-cell subset; the expanded 68-cell audit is pending. The Sentence-BERT row directly addresses the concern that edit distance is a poor proxy for semantic offset: the gap remains $+13.72$~pp. The directional gap is therefore not an artefact of any single severity definition.}

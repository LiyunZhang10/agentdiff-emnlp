...
We package the framework as AgentDiff-Probe v2, a prototype tool that takes a calibration set ($\geq 30$ originals $\times$ 5 perturbations $\times$ 2 scaffolds) and outputs a per-cell $\Delta$ estimate with operator-class breakdown. Leave-one-model-out MAE is $7.10$~pp ($14\%$ over trivial mean) but sign accuracy ($72.2\%$) ties the baseline. We release it as a research-auditing prototype, not a production diagnostic.

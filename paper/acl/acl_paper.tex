...
\paragraph{Held-out within-model results.} Across all 6 cot/react cells of qwen2.5-14B alone, the mean $\Delta$ is $+0.84$~pp ($4/6$ positive). The signal is much smaller than on the original panel ($+14.32$~pp) because qwen2.5-14B is a highly capable instruction-tuned model: its sur-IR is already low on GSM8K cot ($0.113$) and near zero on MATH cot ($0.003$), so even a positive directional gap leaves limited headroom. The held-out validation is therefore best read as a \emph{replication of direction}, not of magnitude: the predicted sign holds in 3 of 4 Group A cells, and the within-Group-A magnitude is $+1.08$~pp; on Group B cells the model exhibits the predicted gate-closure (mean $+0.38$~pp, $1/2$ positive). We report this without rescue: a strong reviewer should read the held-out evidence as confirming \emph{when} the gap appears (capable + tractable), not as inflating \emph{how large} it is.
...
\paragraph{L8. The held-out validation confirms direction but not magnitude.} \S\ref{sec:held_out} reports that on the qwen2.5-14B held-out run the within-Group-A mean $\Delta$ is only $+1.08$~pp ($3/4$ positive), an order of magnitude smaller than the $+13.0$~pp Group A mean on the original panel. Two factors plausibly contribute: (i) qwen2.5-14B is a highly capable instruction-tuned model whose sur-IR is already low on GSM8K cot ($0.113$) and near zero on MATH cot ($0.003$), leaving limited headroom for a positive gap; and (ii) the held-out variants are generated by a different perturbation generator regime ($n{=}200$ pipeline) than the original panel ($n{=}20$--$50$). The held-out evidence is therefore best read as a \emph{replication of direction}, not of magnitude.
...
A fully held-out 7th model run on qwen2.5-14B (1{,}800 trajectories never used to tune the analysis on the original panel) sharpens the picture in two ways. First, it confirms the \emph{direction} of the capability$\times$tractability partition: 3 of the 4 held-out Group A cells are in the predicted direction (the single exception is gsm8k$\times$cot at $\Delta=-2.08$~pp, within the bootstrap 95\% CI bracketing zero), and pooling with the original panel gives Welch $t{=}3.81$, $p{=}9.6\!\times\!10^{-4}$. The within-capable Pearson correlation between accuracy and $\Delta$ collapses to $r{=}+0.13$, so the gate is \emph{threshold-style}, not a linear function of capability --- we retract the linear-monotonicity framing. Second, the held-out trajectories reveal a clean trace-level mechanism we call \textbf{stealth divergence}: a semantic perturbation does not change the agent's first surface action, but corrupts intermediate thought content from step 2 onward (per-step similarity gap $-5.6$ to $-10.5$~pp, paired $t{=}-7.1$ to $-9.1$, $p{<}10^{-11}$) and cascades $0.17$ steps deeper than a presentation-level edit (paired $t{=}7.69$, $p{=}2.5\!\times\!10^{-14}$). Two earlier mechanism claims do not replicate on the held-out model and are explicitly retracted.
